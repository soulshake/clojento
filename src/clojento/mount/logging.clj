; see https://github.com/tolitius/mount/blob/master/test/clj/tapp/utils/logging.clj
(ns clojento.mount.logging
  (:require [mount.core]
...
(defn log-status [f & args]
  (let [[state-name state] args
    ...
    (when-let [taking-over-the-world (whatcha-doing? action)]
      (log/debug (str taking-over-the-world state-name)))
    (apply f args)))
